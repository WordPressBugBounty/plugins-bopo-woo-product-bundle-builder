	...
	class VI_WOO_BOPO_BUNDLE_Frontend {
		...
		function bopobb_show_bundled( $product = null, $mode = 'single' ) {
			...
			if ( $items = $product->get_items() ) {
				...
				if ( isset( $_REQUEST['bopobb_nonce'] ) && wp_verify_nonce( wc_clean( wp_unslash( $_REQUEST['bopobb_nonce'] ) ), 'bopobb_nonce' ) &&
				    isset( $_POST['bopobb_ids'] ) ) {
					$post_ids   = VI_WOO_BOPO_BUNDLE_Helper::bopohp_clean_ids( wp_kses_decode_entities( wp_unslash( $_POST['bopobb_ids'] ) ) );
		...
		function bopobb_display_post_states( $states, $post ) {
			if ( 'product' == get_post_type( $post->ID ) ) {
				if ( ( $_product = wc_get_product( $post->ID ) ) && $_product->is_type( 'bopobb' ) ) {
					...
					$states[] = apply_filters( 'bopobb_post_states', '<span class="bopobb-state">' . sprintf( '%1s (%2s)', esc_html__( 'Bundle', 'woo-bopo-bundle' ), $count ) . '</span>', $count, $_product );
				}
			}
...
//								$cur_key       = substr( $att_k, 10 );
                                $cur_key       = wc_attribute_taxonomy_name( str_replace( 'attribute_pa_', '', urldecode( $att_k ) ) );
							...
							foreach ( $child_wc_variation_attributes as $attr_k => $attr_v ) {
							    $attr_k_decode = urldecode( $attr_k );
								?> data-<?php echo esc_attr( $attr_k_decode ) ?>="<?php echo esc_attr( $attr_v ) ?>"
								<?php
							}
							?> >
...
//				$cur_key  = substr( $att_k, 10 );
		...
		function bopobb_add_to_cart_button() {
			...
			if ( $product && $product->is_type( 'bopobb' ) ) {
				...
				$_nonce = wp_create_nonce('bopobb_nonce');
				echo '<input type="hidden" id="bopobb_nonce" name="bopobb_nonce" value="' . esc_attr( $_nonce ) . '">';
			}
		}
		...
		function bopobb_add_to_cart_validation( $passed, $product_id ) {
			...
			if ( $b_product && $b_product->is_type( 'bopobb' ) ) {
				...
				if ( isset( $_REQUEST['bopobb_ids'] ) ) {
					$ids   = VI_WOO_BOPO_BUNDLE_Helper::bopohp_clean_ids( wp_kses_decode_entities( wp_unslash( $_REQUEST['bopobb_ids'] ) ) );
				...
				if ( ! empty( $items ) ) {
					foreach ( $items as $items_i => $item ) {
						...
						if ( $_product->is_type( 'variable' ) || $_product->is_type( 'bopobb' ) ) {
							wc_add_notice( sprintf('"%1s" %2s',
							    esc_html( $_product->get_name() ), esc_html__( 'is un-purchasable.', 'woo-bopo-bundle' ) ), 'error' );
							wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
						...
						if ( $_product->is_type( 'variation' ) ) {
							...
							if ( $variable_id && $i_product_ids && ! in_array( $variable_id, $i_product_ids ) ) {
								wc_add_notice( sprintf( '"%1s" %2s "%3s".',
								    esc_html( $_product->get_name() ), esc_html__( 'is not bundled of', 'woo-bopo-bundle' ),
								    esc_html( $b_product->get_name() ) ), 'error' );
								wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
							...
							if ( $variable_id && ! self::bopobb_is_bundled( $variable_id, $_variant ) ) {
								wc_add_notice( sprintf('"%1s" %2s',
								    esc_html( $_product->get_name() ), esc_html__( 'is invalid variation.', 'woo-bopo-bundle' ) ), 'error' );
								wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
						...
						if ( $_qty != $items_data['bopobb_bpi_quantity'] ) {
							wc_add_notice( sprintf('"%1s" %2s',
							    ...
							    esc_html__( 'requested quantity is not available in this bundle.', 'woo-bopo-bundle' ) ), 'error' );
							wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
						...
						if ( ! $_product->has_enough_stock( $_qty * $qty ) ) {
							wc_add_notice( sprintf('"%1s" %2s',
							    esc_html( $_product->get_name() ), esc_html__( 'has not enough stock.', 'woo-bopo-bundle' ) ), 'error' );
							wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
						...
						if ( $_product->is_sold_individually() && $this->bopobb_check_in_cart( $_id ) ) {
							wc_add_notice( sprintf( '%1s "%2s" %3s',
							    esc_html__( 'You cannot add another', 'woo-bopo-bundle' ),
							    esc_html( $_product->get_name() ), esc_html__( 'to the cart.', 'woo-bopo-bundle' ) ), 'error' );
							wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
						...
						if ( post_password_required( $_id ) ) {
							wc_add_notice( sprintf( '"%1s" %2s.',
							    esc_html( $_product->get_name() ), esc_html__( 'is protected and cannot be purchased.', 'woo-bopo-bundle' ) ), 'error' );
							wc_add_notice( esc_html__( 'You cannot add this bundle to the cart.', 'woo-bopo-bundle' ), 'error' );
...
//								) );
		...
		function bopobb_before_order_item_meta( $order_item_id ) {
			if ( $parent_id = wc_get_order_item_meta( $order_item_id, '_bopobb_parent_id', true ) ) {
				echo sprintf( '(%1s %2s)', esc_html__( 'bundled in', 'woo-bopo-bundle' ), esc_html( get_the_title( $parent_id ) ) );
			}
		}
		...
		function wp_enqueue_scripts_elementor() {
		    $suffix = WP_DEBUG ? '' : 'min.';
		    wp_enqueue_script( 'woo-bopo-bundle-shortcode-js', VI_WOO_BOPO_BUNDLE_JS . 'shortcode-scripts.' . $suffix . 'js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
		...
		function bopobb_wp_enqueue_scripts() {
			...
			if ( WP_DEBUG ) {
			    wp_enqueue_style( 'woo-bopo-bundle-frontend', VI_WOO_BOPO_BUNDLE_CSS . 'bopo-frontend.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
			    wp_enqueue_style( 'woo-bopo-bundle-icons', VI_WOO_BOPO_BUNDLE_CSS . 'bopo-icon.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
	            wp_enqueue_script( 'woo-bopo-bundle-frontend', VI_WOO_BOPO_BUNDLE_JS . 'bopo-frontend.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
            } else {
			    wp_enqueue_style( 'woo-bopo-bundle-frontend', VI_WOO_BOPO_BUNDLE_CSS . 'bopo-frontend.min.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
			    wp_enqueue_style( 'woo-bopo-bundle-icons', VI_WOO_BOPO_BUNDLE_CSS . 'bopo-icon.min.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
	            wp_enqueue_script( 'woo-bopo-bundle-frontend', VI_WOO_BOPO_BUNDLE_JS . 'bopo-frontend.min.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
            }
