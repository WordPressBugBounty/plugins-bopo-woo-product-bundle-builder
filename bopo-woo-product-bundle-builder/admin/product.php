...
//				wp_reset_postdata();
		...
		function bopobb_default_product() {
			...
			$ex_tag_search  = isset( $_GET['ex_tag'] ) ? array_map('sanitize_text_field', $_GET['ex_tag'] ) : [];
			$prod_search    = isset( $_GET['prod'] ) ? array_map('sanitize_text_field', $_GET['prod'] ) : [];
			$ex_prod_search = isset( $_GET['ex_prod'] ) ? array_map('sanitize_text_field', $_GET['ex_prod'] ) : [];
		...
		function bopobb_product_data_panels() {
            ...
            <div id='bopobb-settings' class='panel woocommerce_options_panel wc-metaboxes-wrapper bopobb-product-panel'>
				<?php wp_nonce_field( 'bopobb_product_nonce', 'bopobb_product_nonce' ); ?>
			...
			for ( $index = 0; $index < $count; $index ++ ) {
                ...
                <div class="bopobb-pbi wc-metabox closed ui-sortable-handle">
                    <h3 class="bopobb-pbi-anchor">
                        <strong class="bopobb-pbi-item-title"><?php echo sprintf( "%1s %2s",
								esc_html__( 'Bundle item', 'woo-bopo-bundle' ), esc_html( $index + 1 ) ); ?></strong>
...
//														}
                                    ...
                                    <div class="bopobb-pbi-input">
                                        <select class="vi-ui fluid dropdown bopo-pbi-input-part"
                                            ...
                                            <option <?php if ( ! empty( $data ) && $data[ $index ]['bopobb_bpi_discount'] == '1' )
												echo esc_attr( 'selected' ) ?>
                                                    value="1"><?php echo sprintf( "%1s (%2s)",
													esc_html__( 'Fixed price', 'woo-bopo-bundle' ),
													esc_html( get_woocommerce_currency_symbol() ) ); ?></option>
                                        </select>
		...
		function bopobb_admin_enqueue_scripts() {
			...
			if ( get_post_type() == 'product' && $screen->id == 'product' ) {
				...
				wp_enqueue_style( 'woo-bopo-bundle-select2-css', VI_WOO_BOPO_BUNDLE_CSS . 'select2.min.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
				wp_enqueue_script( 'jquery-ui-sortable' );
				if ( WP_DEBUG ) {
					wp_enqueue_style( 'woo-bopo-bundle-backend-css', VI_WOO_BOPO_BUNDLE_CSS . 'bopo-settings.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
					wp_enqueue_script( 'woo-bopo-bundle-product-js', VI_WOO_BOPO_BUNDLE_JS . 'bopo-product.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_JS, false );
				} else {
					wp_enqueue_style( 'woo-bopo-bundle-backend-css', VI_WOO_BOPO_BUNDLE_CSS . 'bopo-settings.min.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
					wp_enqueue_script( 'woo-bopo-bundle-product-js', VI_WOO_BOPO_BUNDLE_JS . 'bopo-product.min.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_JS, false );
				}
				...
				wp_enqueue_script( 'woo-bopo-bundle-select2-js', VI_WOO_BOPO_BUNDLE_JS . 'select2.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
