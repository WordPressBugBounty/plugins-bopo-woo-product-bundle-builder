...
<!--                            </tr>-->
                                ...
                                <td>
	                                ...
	                                if ( $this->languages_count ) {
		                                foreach ( $this->languages as $key => $value ) {
			                                ?>
                                            <p>
                                                <label for="<?php echo esc_attr( 'bopobb_popup_title_' . $value ); ?>"><?php
	...
	public function admin_enqueue_script() {
		...
		if ( $page_id == 'woo-bopo-bundle' || $page_id == 'toplevel_page_woo-bopo-bundle' ) {
			// style
			...
			wp_enqueue_style( 'woo-bopo-bundle-semantic-transition-css', VI_WOO_BOPO_BUNDLE_CSS . 'transition.min.css', array(), VI_WOO_BOPO_BUNDLE_VERSION );
			...
			wp_enqueue_script(
				...
				'jquery-ui-slider',
				'jquery-touch-punch'
			), VI_WOO_BOPO_BUNDLE_VERSION, 1
			);
			if ( WP_DEBUG ) {
				wp_enqueue_script( 'woo-bopo-bundle-setting', VI_WOO_BOPO_BUNDLE_JS . 'bopo-settings.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
			} else {
				wp_enqueue_script( 'woo-bopo-bundle-setting', VI_WOO_BOPO_BUNDLE_JS . 'bopo-settings.min.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
			}
			wp_enqueue_script( 'woo-bopo-bundle-semantic-dropdown-js', VI_WOO_BOPO_BUNDLE_JS . 'dropdown.min.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
			wp_enqueue_script( 'woo-bopo-bundle-semantic-transition-js', VI_WOO_BOPO_BUNDLE_JS . 'transition.min.js', array( 'jquery' ), VI_WOO_BOPO_BUNDLE_VERSION, false );
		}
	}
}